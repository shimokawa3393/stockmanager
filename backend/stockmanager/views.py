...
class MainView(APIView):
    ...
    def get(self, request):
        try:
            # ログインユーザーのお気に入り銘柄を取得
            symbols = StockSymbol.objects.filter(user=request.user).values_list("symbol", flat=True)
...
class SaveStockSymbolView(APIView):
    ...
    def post(self, request):
        ...
        if not symbol:
            return Response({"error": "symbolが必要です"}, status=status.HTTP_400_BAD_REQUEST)

        try:
            # 重複登録チェック（任意）
            if StockSymbol.objects.filter(symbol=symbol, user=request.user).exists():
                return Response({"message": "すでに登録されています"}, status=status.HTTP_200_OK)
...
class RemoveStockSymbolView(APIView):
    ...
    def post(self, request):
        ...
        try:
            # 該当のレコードを取得して削除
            favorite = StockSymbol.objects.filter(symbol=symbol, user=request.user).first()
            if favorite:
                favorite.delete()
                ...
                return Response({"message": "登録されていません"}, status=status.HTTP_404_NOT_FOUND)
